// SnakeRewards - Service Worker
// Sin telemetría, sin anuncios, 100% transparente
import { queries } from "/js/queries.js";
import { queriesExtension } from "/js/queriesExtension.js";
import { log, set, get, resetRuntime, verify } from "/js/utils.js";

// Fusionar queries originales con extensión
const allQueries = { ...queries, ...queriesExtension };
...
let config = {
	...
	pro: {
		key: "UNLOCKED",  // PRO siempre desbloqueado
		seats: 999,
	},
};
...
let needPatch = false;
let searchQuery = "";
let shortestDelay = 2000;
let mediumDelay = 5000;
let longestDelay = 8000;
let alive;

// Distribución gaussiana para delays más naturales (anti-detección)
function gaussianRandom(min, max) {
	let u = 0, v = 0;
	while (u === 0) u = Math.random();
	while (v === 0) v = Math.random();
	let num = Math.sqrt(-2.0 * Math.log(u)) * Math.cos(2.0 * Math.PI * v);
	num = (num + 3) / 6; // Normalizar a 0-1
...
async function attach(tabId, interruptible = true) {
	console.log("[SNAKEREWARDS DEBUG] attach() called with tabId:", tabId);
	if (interruptible && !config?.runtime?.running) {
		console.log("[SNAKEREWARDS DEBUG] attach() interrupted - not running");
		logs &&
			log(
	...
	// No confiar en isDebuggerAttached() - siempre intentar adjuntar
	const originalUrl = await getTabUrl(tabId);
	console.log("[SNAKEREWARDS DEBUG] originalUrl:", originalUrl);
	...
	try {
		await race(
			chrome.debugger
				.attach({ tabId }, "1.3")
				.catch((err) => {
					// Si ya hay un debugger adjunto (nuestro o de otro), no es error fatal
					if (
						err.message?.includes("Another debugger") ||
						err.message?.includes("Already attached")
					) {
						console.log("[SNAKEREWARDS DEBUG] Debugger already attached, continuing...");
						log(
							`[ATTACH] - Debugger already attached, continuing...`,
							"warning",
						);
						return; // No lanzar error, continuar
					}
					console.error("[SNAKEREWARDS DEBUG] attach error:", err.message);
					throw err;
				}),
			longestDelay,
		);
		console.log("[SNAKEREWARDS DEBUG] Debugger attached successfully to tab:", tabId);
		logs &&
			log(
...
async function simulate(tabId, interruptible = true) {
	...
	try {
		...
		const deviceMetrics = {
			mobile: true,
			fitWindow: true,
			width: config.device.w + Math.floor(Math.random() * 10) - 5,
			height: config.device.h + Math.floor(Math.random() * 10) - 5,
		...
		// Anti-fingerprint: ocultar navigator.webdriver
		try {
			...
			logs && log(`[SIMULATE] - Anti-fingerprint script injected for tab ${tabId}.`, "success");
		} catch (afError) {
			logs && log(`[SIMULATE] - Anti-fingerprint injection failed: ${afError.message}`, "warning");
		}
		logs &&
			log(
...
async function query(interruptible = true) {
	...
	const categories = Object.keys(allQueries);
	if (niche === "random") {
		niche =
			categories[
	...
	let queryList = allQueries[niche];
	...
	try {
		...
		for (const char of searchQuery) {
			...
			// === TYPING 100% HUMANO ===
			const prevChar = charIndex > 0 ? searchQuery[charIndex - 1] : '';
			const isWordStart = charIndex === 0 || prevChar === ' ';
			const isWordEnd = ' .,:;!?'.includes(char);

			let typingDelay;
			if (isWordStart) {
				// Inicio de palabra: más lento (pensando)
				typingDelay = gaussianRandom(180, 400);
			} else if (isWordEnd) {
				// Fin de palabra/puntuación: pausa larga
				typingDelay = gaussianRandom(250, 600);
			} else {
				// Mitad de palabra: más rápido con variación
				typingDelay = gaussianRandom(60, 200);
			}

			// Micro-pausas aleatorias "pensando" (5% probabilidad)
			if (Math.random() < 0.05) {
				typingDelay += gaussianRandom(400, 1000);
			}
	...
	try {
		...
		try {
			...
			if (
				...
				// SnakeRewards: Sin páginas de afiliados
			}
			if (
				tabs.length > 1 &&
				config?.schedule?.mode !== "m1" &&
		...
		// PRO siempre activo - No promoción
		{
			const modes = {
...
// EnableStats eliminado - Sin telemetría externa
async function enableStats() {
	// SnakeRewards: Sin llamadas a servidores externos
	return;
}

// SnakeRewards: safeBrowsing deshabilitado
// (async () => {
...
chrome.runtime.onInstalled.addListener(async (details) => {
	if (details.reason === "install") {
		log(`[INSTALL] - SnakeRewards installed.`, "update");
		// Sin abrir pestañas externas
	}
	if (details.reason === "update") {
		log(
			`[UPDATE] - SnakeRewards updated to version ${chrome.runtime.getManifest().version
			}.`,
			"update",
		);
		const stored = await get();
